...
class CameraView: UIView, UIGestureRecognizerDelegate {
    ...
    func makeCloseButton() -> UIButton {
        ...
        button.setImage(
            GalleryBundle.image("gallery_close")?.withRenderingMode(.alwaysTemplate),
            for: UIControl.State()
        )
    ...
    func makeFlashButton() -> TripleButton {
        let states: [TripleButton.ButtonState] = [
            TripleButton.ButtonState(
                title: "Gallery.Camera.Flash.Off".g_localize(fallback: "OFF"),
                image: GalleryBundle.image("gallery_camera_flash_off")?.withRenderingMode(.alwaysTemplate) ?? UIImage()
            ),
            TripleButton.ButtonState(
                title: "Gallery.Camera.Flash.On".g_localize(fallback: "ON"),
                image: GalleryBundle.image("gallery_camera_flash_on")?.withRenderingMode(.alwaysTemplate) ?? UIImage()
            ),
            TripleButton.ButtonState(
                title: "Gallery.Camera.Flash.Auto".g_localize(fallback: "AUTO"),
                image: GalleryBundle.image("gallery_camera_flash_auto")?.withRenderingMode(.alwaysTemplate) ?? UIImage()
            )
        ]
    ...
    func makeRotateButton() -> UIButton {
        let button = UIButton(type: .custom)
        button.setImage(
            GalleryBundle.image("gallery_camera_rotate")?.withRenderingMode(.alwaysTemplate),
            for: UIControl.State()
        )
        button.tintColor = Config.Global.tintColor
    ...
    func makeDoneButton() -> UIButton {
        ...
        button.setTitleColor(Config.Camera.BottomContainer.tintColor, for: UIControl.State())
